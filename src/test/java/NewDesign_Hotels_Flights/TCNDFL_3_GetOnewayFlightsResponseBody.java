...
public class TCNDFL_3_GetOnewayFlightsResponseBody extends BaseClass {
	WebDriver driver;  
...
try {	    
      ...
      String actualDate = tripgain_HomePage.getOnlyDayAndMonthFromDateLabel();
      tripgain_HomePage.validateDepartureDateIsDisplayed(actualDate,Log, screenShots);
...
//      System.out.println("Oneway Flight search Body found: " + body1);
...
//
   ...
   // 1. Capture the API response body (Search API)
      String body1 = getResponseBodyByPartialUrl("TGFS&authtoken", 60);

      // Create a reference outside the IF block so we can use it later
      List<Map<String, String>> uiFlightDetails = null;

      if (body1 != null && !body1.isEmpty()) {
          System.out.println("Oneway Flight search Body found: " + body1);
          embedApiResponseInReport("Oneway_FlightSearchAPI", body1);

          // Capture the UI segments ONCE here
          uiFlightDetails = tripgain_HomePage.getDynamicFlightDetails(selectFlightIndex);
          
          // Validate segments
          tripgain_HomePage.validateOnewaySearchDataFromUIAndResponseBody(uiFlightDetails, body1, Log, screenShots);
