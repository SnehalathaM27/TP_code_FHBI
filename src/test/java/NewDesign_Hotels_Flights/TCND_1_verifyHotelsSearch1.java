...
public class TCND_1_verifyHotelsSearch1 extends BaseClass{
	...
	//private final AtomicReference<RequestId> hotelSearchRequestId = new AtomicReference<>();
	//private final AtomicReference<String> hotelSearchApiUrl = new AtomicReference<>();
...
try {	    
	...
	//	NewDesign_HotelsSearchPage.fillRoomDetails("3", "2,3,1", "1,2,0", "5;7,8;", Log);
		...
		hotelSearchApiResponse.set(null);
		
		NewDesign_HotelsSearchPage.clickOnSearchHotelBut();
		Thread.sleep(8000);
		
		try {
		    System.out.println("⏳ Waiting for API response to be fully processed...");
		    
		    WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(60));
		    // This waits until the background listener captures the data
		    wait.until(d -> hotelSearchApiResponse.get() != null);

		    // 1. Get the data from the AtomicReference
		    String finalResponse = hotelSearchApiResponse.get();
		    
		    // 2. THIS IS THE LINE THAT PRINTS THE ENTIRE BODY
		    System.out.println("================= FULL API RESPONSE START =================");
		    System.out.println(finalResponse);
		    System.out.println("================== FULL API RESPONSE END ==================");
		    
		    // 3. (Optional) Verification
		    Assert.assertTrue(finalResponse.contains("hotel"), "Response body does not look like hotel data!");

		} catch (Exception e) {
		    System.err.println("❌ Timeout waiting for API.");
		    throw e;
		}
		
		        // ✅ Read API response
//        String apiResponse = hotelSearchApiResponse.get();
//
//        Assert.assertNotNull(apiResponse, "Hotel Search API response is NULL");
//
//        Log.ReportEvent("INFO", "Hotel Search API response captured successfully");
//
//        System.out.println("\n=========== FINAL API RESPONSE ===========");
	...
	//	NewDesign_Awaiting_ApprovalScreen.validatePriceFromDescWithBookingPage(BookingPgTotalfareAmount,priceFromAwaitingScreen, Log, screenShots);
       ...
       //Function to Logout from Application
    	...
    	if (driver instanceof HasDevTools) {
    	    ...
    	    // Key: RequestId, Value: URL (to verify it's the right one)
    	    java.util.concurrent.ConcurrentHashMap<RequestId, String> pendingRequests = new java.util.concurrent.ConcurrentHashMap<>();
    	    ...
    	    devTools.addListener(Network.responseReceived(), response -> {
    	        ...
    	        if (apiUrl.contains("=TGHS") || apiUrl.contains("hotelsearchv2")) {
    	            System.out.println("🎯 Match Found! Request ID: " + requestId);
    	            
    	            // Use a background thread so we don't block the browser execution
    	            new Thread(() -> {
    	                try {
    	                    // Wait 2 seconds for the data stream to actually finish 
    	                    Thread.sleep(2000); 
    	                    ...
    	                    if (bodyText != null && !bodyText.isEmpty()) {
    	                        hotelSearchApiResponse.set(bodyText);
    	                        System.out.println("✅ BODY CAPTURED! Length: " + bodyText.length());
    	                    } else {
    	                        System.out.println("⚠️ Body was empty for Request ID: " + requestId);
    	                    }
    	                } catch (Exception e) {
    	                    System.out.println("❌ Could not get body yet for " + requestId + ". Error: " + e.getMessage());
    	                }
    	            }).start();
    	        }
    	    });
    	    ...
    	    devTools.addListener(Network.loadingFinished(), loadingFinished -> {
    	        RequestId id = loadingFinished.getRequestId();
    	        
    	        // Only pull the body if this ID was flagged in responseReceived
    	        if (pendingRequests.containsKey(id)) {
    	            try {
    	                // Now that loading is FINISHED, the body is guaranteed to be available
    	                Network.GetResponseBodyResponse body = devTools.send(Network.getResponseBody(id));
    	                String responseBody = body.getBody();
    	                
    	                if (responseBody != null && !responseBody.isEmpty()) {
    	                    hotelSearchApiResponse.set(responseBody);
    	                    System.out.println("✅ Successfully captured body for: " + pendingRequests.get(id));
    	                }
    	            } catch (Exception e) {
    	                System.err.println("❌ Failed to get body for " + id + ": " + e.getMessage());
    	            } finally {
