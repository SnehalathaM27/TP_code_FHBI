...
public class TC_58_VerifyResetofAllFilters extends BaseClass{
	WebDriver driver;    
...
SiteChecker Site_Checker=new SiteChecker(driver);
		...
		Log.ReportEvent("INFO","Before count:");
        trs.validateflightsInitialcount(Log, screenShots);
        Thread.sleep(4000);
        //Function to validate Check In Baggage Functionality on Result Screen
  		...
  		/*trs.validateCheckInBaggageFlightsOnResultScreenRoundTrip(Log, screenShots, index);
  		Thread.sleep(3000);
  		//trs.validatereturncheckinbaggageroundtrip(Log, screenShots, 3);
  		trs.validatereturnCheckInBaggageFlightsOnResultScreenRoundTrip(Log, screenShots, returnindex);
        Thread.sleep(3000);   */   
...
trs.validateFlightsStopsOnResultScreen(numberOfStops,Log, screenShots);
        ...
        //trs.validateroundtripreturnDepartureTimeIsSelected(Log, screenShots, "00 - 06");
       trs.validatereturnFlightsDepartureTimeOnResultScreen(flightStartHour, flightStartMinute, flightEndHour, flightEndMinute, Log, screenShots);
  		...
  		trs.clickclearfiltersRoundtrip();
Thread.sleep(4000);
Log.ReportEvent("INFO","After count:");
trs.validateflightsaftercount(Log, screenShots);
