...
public class TC_25_VerifyCurrencyPersistenceAcrossPages extends BaseClass{
	WebDriver driver;    
...
try {	    
        ...
        String currencyCode = excelData.get("CurrencyCode");
        String expectedCurrency = excelData.get("ExpectedCurrency");
        String expectedCurrencyForPersistence = excelData.get("ExpectedCurrencyForPersistence");
        int fromindex = Integer.parseInt(excelData.get("Fromindex"));
        int toindex = Integer.parseInt(excelData.get("Toindex"));
...
SiteChecker Site_Checker=new SiteChecker(driver);
      ...
      //   tripgainhomepage.searchFlightsOnHomePageroundtrip(Log, screenShots,"blr", "HYD",  fromDate, fromMonthYear, returnDate, returnMonthYear, "Premium Economy", 4);
      ...
      //Function to select Currency Drop Down Values
        tripgainresultspage.selectCurrencyDropDownValues(driver, currencyCode);
        tripgainresultspage.selectFlightBasedOnIndexFromAndReturn(1);
      //Function to validate Currency Value
            
           // trs.validateCurrencyRoundTrip(expectedCurrency,Log,screenShots);
          //  trs.selectFromAndToFlightsBasedOnIndex(fromindex,toindex);
           
    //function for Verify Currency Persistence AcrossPages
        trs.VerifyCurrencyPersistenceAcrossPages(expectedCurrencyForPersistence, Log, screenShots);
    		driver.quit();
