...
public class TC_16_VerifyClearFiltersFunctionality extends BaseClass{
	WebDriver driver;    
...
SiteChecker Site_Checker=new SiteChecker(driver);
        ...
        Thread.sleep(2000);
         ...
         Thread.sleep(3000);
         //    tripgainhomepage.searchFlightsOnHomePageroundtrip(Log, screenShots, "PNQ", "CCU", "23", "May 2025", "29", "May 2025", "Premium Economy",3);
         tripgainhomepage.searchFlightsOnHomePage(Log, screenShots, origin,destination,fromDate,fromMonthYear,returnDate,returnMonthYear,travelClass, adults);
           ...
           Thread.sleep(5000); 
        trs.verifyFlightsDetailsOnResultScreenForLocal(index, Log, screenShots);
        Thread.sleep(7000); 
        
        
        double[] values=trs.defaultPriceRangeOfSlider(driver,Log,screenShots);
        double defaultMinValue=values[0];
        double defaultMaxValue=values[1];
        double maximumValue=defaultMaxValue-3000;
        trs.adjustMaximumSliderToValue(driver,maximumValue);
        trs.verifyPriceRangeValuesOnResultScreen(Log,screenShots);
        Thread.sleep(3000);

        trs.clearfilter(Log,screenShots);
        Thread.sleep(3000);

        double[] newValues=trs.defaultPriceRangeOfSlider(driver,Log,screenShots);
        double newMinValue=newValues[0];
        double newMaxValue=newValues[1];
        
        
         trs.validateDefaultAndLatestPriceRangeValuesOnSlideBarForRoundWay(Log,screenShots,defaultMinValue,defaultMaxValue,newMinValue,newMaxValue);
