...
public class TC_14_VerifyPriceFilterAdjustment_MinValue extends BaseClass{
	WebDriver driver;    
...
SiteChecker Site_Checker=new SiteChecker(driver);
        ...
        double [] value =trs.defaultPriceRangeOfSlider(driver,Log,screenShots);
             Thread.sleep(2000);
             
             double values[] =trs.defaultPriceRangeOfSlider(driver,Log,screenShots);
             double minValue=values[0];
             double minimumValue=minValue+3000;
             trs.adjustMinimumSliderToValue(driver,30000);
             //tripgain_roundtrip_resultpage.verifyPriceRangeValuesOnResultScreen(Log,screenShots,0);
